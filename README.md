# Hi! I'm Antti Rask

I'm a data person from Finland. I love working with data, learning new things, and sharing what I learn along the way.

## What I Do

- **Data Analysis & Visualization**, mainly with R, ggplot2, and the rest of the tidyverse
- **Shiny Apps** for interactive data applications
- **Power BI & DAX** for business intelligence
- **Machine Learning** using tidymodels
- Nowadays I also use **Claude Code** (like everyone else and their grandma) and **Python** for some of the projects
  - While I don't think that **AI** is the answer to all our problems, it does make coding easier/faster
  - And as mentioned, I like learning new things, which is another reason I've kept an open mind

## Featured Projects

### Applications

| Project | Website | Source | Description |
| ------- | ------- | ------ | ----------- |
| [BiblioStatus](https://youcanbeapirate.com/BiblioStatus/) | [on youcanbeapiRate.com](https://bibliostatus.youcanbeapirate.com/) | [GitHub](https://github.com/AnttiRask/BiblioStatus/) | Finnish library opening hours on a map and in real-time |
| [TrackTeller](https://youcanbeapirate.com/TrackTeller/) | [on youcanbeapiRate.com](https://trackteller.youcanbeapirate.com/) | [GitHub](https://github.com/AnttiRask/TrackTeller/) | Spotify listening insights and playlist creator |
| [TuneTeller](https://youcanbeapirate.com/TuneTeller/) | [on youcanbeapiRate.com](https://tuneteller.youcanbeapirate.com/) | [GitHub](https://github.com/AnttiRask/TuneTeller/) | Artist recommendations using OpenAI + Spotify APIs. Honorable mention in the Posit 2024 Shiny Contest |
| [Gallery of the Day](https://youcanbeapirate.com/gallery-of-the-day/) | [on youcanbeapiRate.com](https://galleryoftheday.youcanbeapirate.com/) | [GitHub](https://github.com/AnttiRask/gallery-of-the-day/) | Daily AI-generated images inspired by historical events |

### Books & Educational Content

| Source | Description |
| ------ | ----------- |
| [ggplot2 extended](https://github.com/AnttiRask/ggplot2-extended-book) | My [book](https://ggplot2-extended-book.com/) (in progress) on extending ggplot2 capabilities |
| [How to {insert topic here} with R](https://github.com/AnttiRask/how_to_insert_topic_here_with_R) | Collection of R tutorials and examples |
| [Learning DAX](https://github.com/AnttiRask/learning_dax) | DAX patterns translated to VAR/RETURN syntax |
| [Learning ggplot2](https://github.com/AnttiRask/learning_ggplot2) | Comprehensive ggplot2 learning resources |
| [Learning Machine Learning](https://github.com/AnttiRask/learning_machine_learning) | ML with tidyverse and tidymodels |

### R Packages & Tools

| Project | Description |
| ------- | ----------- |
| [col2hex2col](https://youcanbeapirate.com/col2hex2col/) | R package for converting between color names and hex codes (author) |
| [RandomWalker](https://www.spsanderson.com/RandomWalker/) | R package for generating, visualizing, and analyzing random walks (co-author) |

### Data Visualization

| Source | Description |
| ------ | ----------- |
| [Hillshade Finland](https://github.com/AnttiRask/hillshade_finland) | Hillshade maps of Finland |
| [Starmap with Starbliss](https://github.com/AnttiRask/starmap_with_starbliss) | Creating beautiful star maps with R |

## Tech Stack

![Claude Code](https://img.shields.io/badge/-Claude%20Code-D4A574?style=flat-square&logo=anthropic&logoColor=white)
![Docker](https://img.shields.io/badge/-Docker-2496ED?style=flat-square&logo=docker&logoColor=white)
![Google Cloud](https://img.shields.io/badge/-Google%20Cloud-4285F4?style=flat-square&logo=googlecloud&logoColor=white)
![Power BI](https://img.shields.io/badge/-Power%20BI-F2C811?style=flat-square&logo=powerbi&logoColor=black)
![Python](https://img.shields.io/badge/-Python-3776AB?style=flat-square&logo=python&logoColor=white)
![R](https://img.shields.io/badge/-R-276DC3?style=flat-square&logo=r&logoColor=white)
![RStudio](https://img.shields.io/badge/-RStudio-75AADB?style=flat-square&logo=rstudio&logoColor=white)
![Shiny](https://img.shields.io/badge/-Shiny-13B5EA?style=flat-square&logo=r&logoColor=white)
![Snowflake](https://img.shields.io/badge/-Snowflake-29B5E8?style=flat-square&logo=snowflake&logoColor=white)

## Blog(s)

- I have two Substack channels:
  - [ggplot2 extended](https://ggplot2extended.substack.com/) is for all the R stuff, including BTS for the book I'm writing
  - [youcanbeapiRate](https://youcanbeapirate.substack.com/) is for everything else I'm interested in
- My portfolio site: [youcanbeapiRate.com](https://www.youcanbeapirate.com/)

## Connect

Feel free to reach out on [LinkedIn](https://www.linkedin.com/in/anttirask/) or explore my repositories!
